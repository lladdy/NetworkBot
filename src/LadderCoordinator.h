/*
	This is a modified copy of the s2client-api Coordinator in order to allow it to connect over a network.
*/
...
public:
    ...
    void SetTimeoutMS(uint32_t timeout_ms = sc2::kDefaultProtoInterfaceTimeout);
    ...
    void SetFeatureLayers(const sc2::FeatureLayerSettings& settings);

    //!
    //! \sa RenderSettings
    void SetRender(const sc2::RenderSettings& settings);
    ...
    // \sa PlayerSetup
    void SetParticipants(const std::vector<sc2::PlayerSetup>& participants);
    ...
    // \sa ReplayObserver
    void AddReplayObserver(sc2::ReplayObserver* replay_observer);
	...
	//! Attaches to a running Starcraft on the local machine.
	void Connect(int port);

	//! Attaches to a running Starcraft at the network address.

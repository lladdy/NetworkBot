...
void RunParallel(const std::function<void(sc2::Agent* a)>& step, std::vector<sc2::Agent*>& agents) {
    // Run all steps in parallel.
    std::vector<std::thread> threads(agents.size());
    for (size_t i = 0; i < agents.size(); ++i) {
		sc2::Agent* a = agents[i];
...
int LaunchProcess(sc2::ProcessSettings& process_settings, sc2::Client* client, int window_width, int window_height, int window_start_x, int window_start_y, int port, int client_num=0) {
    assert(client);
    process_settings.process_info.push_back(sc2::ProcessInfo());
	sc2::ProcessInfo& pi = process_settings.process_info.back();
    ...
    pi.process_id = sc2::StartProcess(process_settings.process_path, cl);
    if (!pi.process_id) {
...
bool AttachClients(sc2::ProcessSettings& process_settings, std::vector<sc2::Client*> clients) {
    bool connected = false;

    // Since connect is blocking do it after the processes are launched.
    for (std::size_t i = 0; i < clients.size(); ++i) {
        const sc2::ProcessInfo& pi = process_settings.process_info[i];
		sc2::Client* c = clients[i];
...
int LaunchProcesses(sc2::ProcessSettings& process_settings, std::vector<sc2::Client*> clients, int window_width, int window_height, int window_start_x, int window_start_y) {
    int last_port = 0;
...
static void CallOnStep(sc2::Agent* a) {
	sc2::ControlInterface* control = a->Control();
    if (!control->IsInGame()) {
        a->OnGameEnd();
	...
	sc2::ActionInterface* action = a->Actions();
    control->IssueEvents(action->Commands());
    if (action) {
        action->SendActions();
    }

	sc2::ActionFeatureLayerInterface* action_feature_layer = a->ActionsFeatureLayer();
...
// Implementation.
class LadderCoordinatorImp {
public:
    std::vector<sc2::Agent*> agents_;
    std::vector<sc2::ReplayObserver*> replay_observers_;

    bool game_ended_;
    bool starcraft_started_;

	sc2::GameSettings game_settings_;
	sc2::ReplaySettings replay_settings_;
    ...
    bool JoinGame();
    void StartReplay();
    bool ShouldIgnore(sc2::ReplayObserver* r, const std::string& file);
    bool ShouldRelaunch(sc2::ReplayObserver* r);
    ...
    void AddAgent(sc2::Agent* agent);

    bool Relaunch(sc2::ReplayObserver* replay_observer);
...
LadderCoordinatorImp::~LadderCoordinatorImp() {
    for (auto& p : process_settings_.process_info) {
		sc2::TerminateProcess(p.process_id);
    }
}

bool LadderCoordinatorImp::AnyObserverAvailable() const {
    return std::any_of(replay_observers_.cbegin(), replay_observers_.cend(),
                       [](sc2::ReplayObserver* r) {
                           return !r->Control()->IsInGame();
                       });
}

bool LadderCoordinatorImp::ShouldIgnore(sc2::ReplayObserver* r, const std::string& file) {
    if (file.empty())
        return true;
...
bool LadderCoordinatorImp::ShouldRelaunch(sc2::ReplayObserver* r) {
    const sc2::ReplayInfo& replay_info = r->ReplayControl()->GetReplayInfo();
    ...
    if (!sc2::FindBaseExe(process_settings_.process_path, replay_info.base_build))
        return false;
    ...
    r->Control()->Error(sc2::ClientError::WrongGameVersion);
    return true;
}

void LadderCoordinatorImp::StartReplay() {
...
void LadderCoordinatorImp::StepAgents() {
    auto step_agent = [this](sc2::Agent* a) {
		sc2::ControlInterface* control = a->Control();

        if (control->GetAppState() != sc2::AppState::normal) {
            return;
        }
    ...
    if (!process_settings_.multi_threaded) {
        for (auto a : agents_) {
            if (a->Control()->GetAppState() != sc2::AppState::normal) {
                continue;
            }
...
void LadderCoordinatorImp::StepAgentsRealtime() {
    auto step_agent = [](sc2::Agent* a) {
		sc2::ControlInterface* control = a->Control();
        if (!control) {
            return;
        }
...
void LadderCoordinatorImp::StepReplayObservers() {
    // Run all replay observers.
    auto run_replay = [this](sc2::ReplayObserver* r) {
        if (r->Control()->GetAppState() != sc2::AppState::normal) {
            return;
        }
...
void LadderCoordinatorImp::StepReplayObserversRealtime() {
    // Run all replay observers.
    auto run_replay = [this](sc2::ReplayObserver* r) {
        if (r->Control()->GetAppState() != sc2::AppState::normal) {
            return;
...
bool LadderCoordinatorImp::WaitForAllResponses() {
    ...
    for (;;) {
        ...
        for (sc2::Agent* agent : agents_) {
            if (!agent->Control()->HasResponsePending() || agent->Control()->GetAppState() != sc2::AppState::normal) {
                continue;
            }
        ...
        for (sc2::ReplayObserver* replay_observer : replay_observers_) {
            if (has_responses) {
                break;
            }

            if (!replay_observer->Control()->HasResponsePending() || replay_observer->Control()->GetAppState() != sc2::AppState::normal) {
                continue;
            }
            has_responses = true;
...
bool LadderCoordinatorImp::JoinGame() {
    ...
    for (auto c : agents_) {
		sc2::ControlInterface* control = c->Control();
        const std::vector<sc2::ClientError>& client_errors = control->GetClientErrors();
...
bool LadderCoordinatorImp::StartGame() {
...
bool LadderCoordinatorImp::Relaunch(sc2::ReplayObserver* replay_observer) {
	sc2::ControlInterface* control = replay_observer->Control();
    const sc2::ProcessInfo& pi = control->GetProcessInfo();
    ...
    const sc2::ProcessInfo& pi_new = control->GetProcessInfo();
...
bool LadderCoordinator::StartGame(const std::string& map_path) {
    if (!map_path.empty())
        imp_->game_settings_.map_name = map_path;
    return imp_->StartGame();
}

bool LadderCoordinator::JoinGame() {
    return imp_->JoinGame();
}

bool LadderCoordinator::CreateGame(const std::string& map_path) {
    if (!map_path.empty())
        imp_->game_settings_.map_name = map_path;
    return imp_->CreateGame();
}

bool LadderCoordinator::RemoteSaveMap(const void* data, int data_size, std::string remote_path) {
...
void LadderCoordinator::SetParticipants(const std::vector<sc2::PlayerSetup>& participants) {
...
void LadderCoordinator::SetReplayRecovery(bool value) {
    imp_->replay_recovery_ = value;
}


bool LadderCoordinator::LoadSettings(int argc, char** argv) {
    return ParseSettings(argc, argv, imp_->process_settings_, imp_->game_settings_);
}

void LadderCoordinator::LaunchStarcraft() {
    if (!sc2::DoesFileExist(imp_->process_settings_.process_path)) {
...
void LadderCoordinator::Connect(int port) {
	Connect(imp_->process_settings_.net_address, port);
}

void LadderCoordinator::Connect(const std::string& address, int port) {
	while (imp_->process_settings_.process_info.size() < imp_->agents_.size()) {
		imp_->process_settings_.process_info.push_back(
			sc2::ProcessInfo(address, 0, port)
		);
	}
...
void LadderCoordinator::LeaveGame() {
    for (auto c : imp_->agents_) {
        c->Control()->RequestLeaveGame();
    }
}

void LadderCoordinator::WaitForAllResponses() {
    imp_->WaitForAllResponses();
}

bool LadderCoordinator::Update() {
    ...
    for (auto agent : imp_->agents_) {
        const sc2::ControlInterface* control = agent->Control();
        const std::vector<sc2::ClientError>& client_errors = control->GetClientErrors();
        if (!client_errors.empty()) {
...
bool LadderCoordinator::AllGamesEnded() const {
...
void LadderCoordinatorImp::AddAgent(sc2::Agent* agent) {
    assert(agent);
    agents_.push_back(agent);
}

void LadderCoordinator::AddReplayObserver(sc2::ReplayObserver* replay_observer) {
    assert(replay_observer);
    imp_->replay_observers_.push_back(replay_observer);
}

void LadderCoordinator::SetMultithreaded(bool value) {
    imp_->process_settings_.multi_threaded = value;
}

void LadderCoordinator::SetRealtime(bool value) {
    // Realtime must be set before LaunchStarcraft is called.
    assert(!imp_->starcraft_started_);
    imp_->process_settings_.realtime = value;
}

void LadderCoordinator::SetStepSize(int step_size) {
    if (step_size < 1) {
        assert(0);
        return;
    }
...
void LadderCoordinator::SetProcessPath(const std::string& path) {
    assert(!imp_->starcraft_started_);
    imp_->process_settings_.process_path = path;
}

void LadderCoordinator::SetDataVersion(const std::string& version) {
    assert(!imp_->starcraft_started_);
    imp_->process_settings_.data_version = version;
}

void LadderCoordinator::SetTimeoutMS(uint32_t timeout_ms) {
    imp_->process_settings_.timeout_ms = timeout_ms;
}

void LadderCoordinator::SetPortStart(int port_start) {
    assert(!imp_->starcraft_started_);
    imp_->process_settings_.port_start = port_start;
}

void LadderCoordinator::SetFeatureLayers(const sc2::FeatureLayerSettings& settings) {
...
void LadderCoordinator::SetRender(const sc2::RenderSettings& settings) {
...
void LadderCoordinator::SetWindowSize(int width, int height) {
    assert(!imp_->starcraft_started_);
    imp_->window_width_ = width;
    imp_->window_height_ = height;
}

void LadderCoordinator::SetWindowLocation(int x, int y) {
    assert(!imp_->starcraft_started_);
    imp_->window_start_x_ = x;
    imp_->window_start_y_ = y;
}

void LadderCoordinator::SetUseGeneralizedAbilityId(bool value) {
    assert(!imp_->starcraft_started_);
    imp_->use_generalized_ability_id = value;
}

bool LadderCoordinator::SetReplayPath(const std::string& path) {
    imp_->replay_settings_.replay_file.clear();

    if (sc2::HasExtension(path, ".SC2Replay")) {
        ...
        if (!imp_->replay_settings_.replay_dir.empty()) {
			sc2::scan_directory(imp_->replay_settings_.replay_dir.c_str(), imp_->replay_settings_.replay_file, true);
        }
    }
...
bool LadderCoordinator::LoadReplayList(const std::string& file_path) {
...
void LadderCoordinator::SaveReplayList(const std::string& file_path) {
...
bool LadderCoordinator::HasReplays() const {
    return !imp_->replay_settings_.replay_file.empty();
}

void LadderCoordinator::AddCommandLine(const std::string& option) {
    imp_->process_settings_.extra_command_lines.push_back(option);
}

std::string LadderCoordinator::GetExePath() const {
...
void LadderCoordinator::SetupPorts(size_t num_agents, int port_start, bool check_single) {
